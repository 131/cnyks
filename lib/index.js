...
class Cnyks {
  ...
  */ {
    ...
    if(Object.keys(command['usage']['params']).length) {
      ...
      for(let [param_name, param_infos] of Object.entries(command['usage']['params'])) {
        ...
        tmp_str.push((param_infos['optional'] ? '[' : '') + (param_infos['rest'] ? '...' : '') + "$" +  param_name);
      }
  ...
  */ {
  ...
  _respond(response) {
    ...
    if(!this.output)
      return this._stderr(this._box("Response", response));

    if(Buffer.isBuffer(response))
      return this._stdout(response);

    if(this.output == OUTPUT_RAW)
      return this._stdout(String(response));

    if(this.output == OUTPUT_JSON)
      return this._stdout(JSON.stringify(response) + "\n");
  }
